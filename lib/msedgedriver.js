...
process.env.PATH = path.join(__dirname, "msedgedriver") + path.delimiter + process.env.PATH;
exports.path =
  process.platform === "win32"
    ? path.join(__dirname, "msedgedriver", "msedgedriver.exe")
    : path.join(__dirname, "msedgedriver", "msedgedriver");
exports.version = "87.0.664.47";
exports.start = function(args, returnPromise) {
  let command = exports.path;
  if (!fs.existsSync(command)) {
    console.log("Could not find msedgedriver in default path: ", command);
    console.log("Falling back to use global msedgedriver bin");
    command = process.platform === "win32" ? "msedgedriver.exe" : "msedgedriver";
  }
