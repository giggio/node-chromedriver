...
let chromedriver_version = process.env.npm_config_chromedriver_version || process.env.CHROMEDRIVER_VERSION || exports.version;
...
exports.download = async function (options) {
  ...
  try {
    ...
    chromedriver_version = await exports.getChromeDriverVersionFromUrl(`${cdnUrl}/LATEST_RELEASE_${chromeVersionWithoutPatch}`);
    ...
    if (latestReleaseForVersionMatch) {
      ...
      chromedriver_version = await exports.getChromeDriverVersionFromUrl(`${cdnUrl}/LATEST_RELEASE_${majorVersion}`);
    }
    let tmpPath = findSuitableTempDirectory(chromedriver_version);
    let chromedriverIsAvailable = await verifyIfChromedriverIsAvailableAndHasCorrectVersion(chromedriver_version);
    if (!chromedriverIsAvailable || options.force_download === 'true') {
      ...
      await downloadChromedriver(cdnUrl, chromedriver_version, tmpPath);
...
function fixFilePermissions() {
  ...
  if (process.platform != 'win32') {
    const stat = fs.statSync(exports.path);
